...
We fit a second linear model that included all of the above predictors with log(selling_price) as the outcome variable and log(km_driven) as one of the predictors.

```{r echo=FALSE}
fit.log <- lm(log(selling_price) ~ log(km_driven)+seller_type+transmission+owner+mileage+engine+max_power+seats, data = data.df)
log.vals <- coef(summary(fit.log))
colnames(log.vals)[4] <- "p value"
log.vals <- round(log.vals, 4)
kable(log.vals)
```

The adjusted R-squared value for this model was 0.7501, meaning that around 75% of the variation of selling price can be explained by the predictors included in the model. This surpasses the previous model and is likely due to the fact that by taking the logarithm of data with a large range of values, we reduce the skewness of the data and stabilize the variance.

The following is a residual histogram and a Q-Q plot to show how closely our residuals follow a normal distribution. The histogram displays a strong peak slightly right of the center with evident skewness and a few big outliers (although the outliers aren't as apparent as previously). The same is also reflected in the Q-Q plot, indicating that the model is not perfectly normal but more normal than the previous model.
